/** Wavex-os wavex-os Fastify route registrar. Mounts all wavex-os
 *  onboarding endpoints under /wavex-os/onboarding/* against an existing
 *  Fastify instance.
 *
 ...
 *  - WAVEX_OS_CLAUDE_BIN is set so tier-router T2 calls reach the operator's
...
import { registerPlanAssemblyRoutes } from "./routes/plan-assembly.js";
import { registerAdoptProductRoute } from "./routes/adopt-product.js";
import { registerApproveOrganizationRoute } from "./routes/approve-organization.js";
...
export function registerWavexOsRoutes(app: FastifyInstance): void {
  bootstrap();
  registerPillarRoutes(app);
  registerPhaseRoutes(app);
  registerPlanAssemblyRoutes(app);
  registerAdoptProductRoute(app);
  registerApproveOrganizationRoute(app);
